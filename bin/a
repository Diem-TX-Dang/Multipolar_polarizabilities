...
!(/  (/1,4,5/), (/ 4,2,6 /)  , (/ 5,6,3 /)   /)
  ...
  ! Diem Dang and Nam Le modified 
  ! Independent components of a rank-3 symmetric tensor
  !
  ! 1 <--> xxx
  ! 2 <--> yyy
  ! 3 <--> zzz
  ...
  !10 <--> xyz
  !
  integer, dimension(10), parameter ::  alpha_P = (/1, 2, 3, 1, 1, 2, 1, 1, 2, 1/)
  integer, dimension(10), parameter ::  beta_P  = (/1, 2, 3, 1, 1, 2, 2, 3, 3, 2/)
  integer, dimension(10), parameter ::  gamma_P = (/1, 2, 3, 2, 3, 3, 2, 3, 3, 3/)  
  !
...
contains
  ...
  subroutine berry_main
    ...
    ! Diem Dang and Nam Le modify to export
    ! dipole - dipole polarizability
    complex(kind=dp), allocatable :: didi_polar_k_list(:, :, :)
    complex(kind=dp), allocatable :: didi_polar_list(:, :, :)
    ! dipole - quadrupole polarizability
    complex(kind=dp), allocatable :: diquad_polar_k_list(:, :, :, :)
    complex(kind=dp), allocatable :: diquad_polar_list(:, :, :, :)
    ! quadrupole - quadrupole polarizability
    complex(kind=dp), allocatable :: quadquad_polar_k_list(:, :, :, :, :)
    complex(kind=dp), allocatable :: quadquad_polar_list(:, :, :, :, :)    
    ! dipole - octupole polarizability
    complex(kind=dp), allocatable :: dioctu_polar_k_list(:, :, :, :, :)
    complex(kind=dp), allocatable :: dioctu_polar_list(:, :, :, :, :)        
    ...
    integer           :: n, i, j, k, l, m, jk, ilm, ikpt, if, ispn, ierr, loop_x, loop_y, loop_z, &
    ...
    logical           :: eval_ahc, eval_morb, eval_kubo, not_scannable, eval_sc, eval_shc, &
                         eval_polar ! Diem Dang and Nam Le
    logical           :: ladpt_kmesh
    ...
    eval_sc = .false.
    eval_shc = .false.
    !eval_polar = .false. !Diem Dang and Nam Le
    ...
    if (eval_polar) then
      call get_HH_R
      call get_AA_R
      allocate (didi_polar_k_list(3, 3, kubo_nfreq))
      allocate (didi_polar_list(3, 3, kubo_nfreq))
      didi_polar_k_list = cmplx_0
      didi_polar_list = cmplx_0
      allocate (diquad_polar_k_list(3, 3, 3, kubo_nfreq))
      allocate (diquad_polar_list(3, 3, 3, kubo_nfreq))
      diquad_polar_k_list = cmplx_0
      diquad_polar_list = cmplx_0
      allocate (quadquad_polar_k_list(3, 3, 3, 3, kubo_nfreq))
      allocate (quadquad_polar_list(3, 3, 3, 3, kubo_nfreq))
      quadquad_polar_k_list = cmplx_0
      quadquad_polar_list = cmplx_0
      allocate (dioctu_polar_k_list(3, 3, 3, 3, kubo_nfreq))
      allocate (dioctu_polar_list(3, 3, 3, 3, kubo_nfreq))
      dioctu_polar_k_list = cmplx_0
      dioctu_polar_list = cmplx_0
    endif
    ...
    if (on_root) then
      ...
      if (eval_polar) write (stdout, '(/,3x,a)') &
        '* Multipole - Multipole Polarizability'
    ...
    if (wanint_kpoint_file) then
      ...
      do loop_xyz = 1, num_int_kpts_on_node(my_node_id)
        ...
        ! Diem Dang and Nam Le
        if (eval_polar) then
          call berry_get_polar_klist(kpt, didi_polar_k_list, diquad_polar_k_list, quadquad_polar_k_list, dioctu_polar_k_list)
          didi_polar_list = didi_polar_list + didi_polar_k_list*kweight
          diquad_polar_list = diquad_polar_list + diquad_polar_k_list*kweight
          quadquad_polar_list = quadquad_polar_list + quadquad_polar_k_list*kweight
          dioctu_polar_list = dioctu_polar_list + dioctu_polar_k_list*kweight
        end if
        !
    ...
    ! Diem Dang and Nam Le
    if (eval_polar) then
      call comms_reduce(didi_polar_list(1, 1, 1), 3*3*kubo_nfreq, 'SUM')
      call comms_reduce(diquad_polar_list(1, 1, 1, 1), 3*3*3*kubo_nfreq, 'SUM')
      call comms_reduce(quadquad_polar_list(1, 1, 1, 1, 1), 3*3*3*3*kubo_nfreq, 'SUM')
      call comms_reduce(dioctu_polar_list(1, 1, 1, 1, 1), 3*3*3*3*kubo_nfreq, 'SUM')
    end if
    ...
    if (on_root) then
      ...
      if (eval_polar) then
        ...
        didi_polar_list = fac1*didi_polar_list ! convert alpha to Farad/Angstrom
        diquad_polar_list = fac1*diquad_polar_list !convert \mathbb{A} to Farad
        quadquad_polar_list = fac1*quadquad_polar_list !convert \mathbb{C} to Farad.Angstrom
        dioctu_polar_list = fac1*dioctu_polar_list !convert \mathbb{D} to Farad.Angstrom        
        do i = 1, 3
          do j = 1, 3
            file_name = trim(seedname)//'-polarizability-dd_'// &
                        achar(119 + i)//achar(119 + j)//'.dat'
            file_name = trim(file_name)
            file_unit = io_file_unit()
            open (file_unit, FILE=file_name, STATUS='UNKNOWN', FORM='FORMATTED')
            ...
            do ifreq = 1, kubo_nfreq
              ...
              real(kubo_freq_list(ifreq), dp), didi_polar_list(i, j, ifreq)
            enddo
            close (file_unit)
            do k = 1, 3
              file_name = trim(seedname)//'-polarizability-dq_'// &
                          achar(119 + i)//achar(119 + j)//achar(119 + k)//'.dat'
              file_name = trim(file_name)
              file_unit = io_file_unit()
              open (file_unit, FILE=file_name, STATUS='UNKNOWN', FORM='FORMATTED')
              ...
              do ifreq = 1, kubo_nfreq
                write (file_unit, '(1E16.8, 2E16.8)') &
                real(kubo_freq_list(ifreq), dp), diquad_polar_list(i, j, k, ifreq)
              enddo
              close (file_unit)
              do l = 1, 3
                file_name = trim(seedname)//'-polarizability-qq_'// &
                ...
                do ifreq = 1, kubo_nfreq
                  write (file_unit, '(1E16.8, 2E16.8)') &
                  real(kubo_freq_list(ifreq), dp), quadquad_polar_list(i, j, k, l, ifreq)
                enddo
                close (file_unit)
                file_name = trim(seedname)//'-polarizability-do_'// &
                            achar(119 + i)//achar(119 + j)//achar(119 + k)//achar(119 + l)//'.dat'
                file_name = trim(file_name)
                file_unit = io_file_unit()
                open (file_unit, FILE=file_name, STATUS='UNKNOWN', FORM='FORMATTED')
                ...
                do ifreq = 1, kubo_nfreq
                  write (file_unit, '(1E16.8, 2E16.8)') &
                  real(kubo_freq_list(ifreq), dp), dioctu_polar_list(i, j, k, l, ifreq)
                enddo
                close (file_unit)
              enddo  
            enddo
...
!             delta=delta/eta_smr
  ...
  subroutine berry_get_polar_klist(kpt, didi_polar_k_list, diquad_polar_k_list, quadquad_polar_k_list, dioctu_polar_k_list)
    !====================================================================!
    !  Diem Dang and Nam Le                                              !
    !  Contribution from point k to the Npole-Npole polarazibility       !
    ...
    use w90_io, only: io_error, stdout, io_file_unit, seedname, &
      io_stopwatch, io_date ! Diem Dang and Nam Le modify to calculate polarizability alpha
    use w90_kronecker_delta
    ! Arguments
    !
    real(kind=dp), intent(in)                           :: kpt(3)
    complex(kind=dp), intent(out), dimension(:, :, :)   :: didi_polar_k_list
    complex(kind=dp), intent(out), dimension(:, :, :, :)   :: diquad_polar_k_list
    complex(kind=dp), intent(out), dimension(:, :, :, :, :)   :: quadquad_polar_k_list
    complex(kind=dp), intent(out), dimension(:, :, :, :, :)   :: dioctu_polar_k_list
    ...
    complex(kind=dp), allocatable :: HH(:, :)
    complex(kind=dp), allocatable :: D_h(:, :, :)
    complex(kind=dp), allocatable :: r_dp(:, :, :)
    complex(kind=dp), allocatable :: rr(:, :, :, :)
    complex(kind=dp), allocatable :: rrr(:, :, :, :, :)
    real(kind=dp), allocatable    :: eig(:)
    real(kind=dp), allocatable    :: eig_da(:, :)
    real(kind=dp), allocatable    :: occ(:)

    complex(kind=dp)              :: sum_AD(3, 3), sum_HD(3, 3), gen_r_nm(3)
    integer                       :: i, if, a, b, c, e, bc, d, n, m, k, l, r, ifreq, istart, iend
    ...
    allocate (r_dp(num_wann, num_wann, 3))
    allocate (rr(num_wann, num_wann, 3, 3))
    allocate (rrr(num_wann, num_wann, 3, 3, 3))
    ...
    ! Diem Dang and Nam Le modify to calculate contribution of dipole - quadrupole, quadrupole - quadrupole, dipole - octupole polarizability A at k point  
    ! dipole matrix element  
    r_dp(:, :, :) = AA_bar(:, :, :) + cmplx_i * D_h(:, :, :) 
    ...
    do n = 1, num_wann
      do m = 1, num_wann
        ...
        do k = 1, num_wann 
          if (k == n .or. k == m) cycle
          !cycle bands above the maximum
          if (eig(k) > kubo_eigval_max) cycle
          do a = 1, 3
            do b = 1, 3
              rr(n, m, a, b) = rr(n, m, a, b) + 0.75_dp * (r_dp(n, k, a) * r_dp(k, m, b) + r_dp(n, k, b) * r_dp(k, m, a)) - 0.5_dp * kronecker_delta(a, b) * (r_dp(n, k, 1) * r_dp(k, m, 1) + r_dp(n, k, 2) * r_dp(k, m, 2) + r_dp(n, k, 3) * r_dp(k, m, 3))
            enddo
          enddo
          do l = 1, num_wann
            if (l == n .or. l == m .or. l == k) cycle
            !cycle bands above the maximum
            if (eig(l) > kubo_eigval_max) cycle  
            do a = 1, 3
              do b = 1, 3 
                do c = 1, 3
                  rrr(n, m, a, b, c) = rrr(n, m, a, b, c) + 0.41666666667_dp * (r_dp(n, k, a) * r_dp(k, l, b) * r_dp(l, m, c) + r_dp(n, k, a) * r_dp(k, l, c) * r_dp(l, m, b) + r_dp(n, k, b) * r_dp(k, l, a) * r_dp(l, m, c) + r_dp(n, k, b) * r_dp(k, l, c) * r_dp(l, m, a) + r_dp(n, k, c) * r_dp(k, l, a) * r_dp(l, m, b) + r_dp(n, k, c) * r_dp(k, l, b) * r_dp(l, m, a)) &
                                                          - 0.16666666667_dp * (r_dp(n, k, 1) * r_dp(k, l, 1) + r_dp(n, k, 2) * r_dp(k, l, 2) + r_dp(n, k, 3) * r_dp(k, l, 3)) * (kronecker_delta(b, c) * r_dp(l, m, a) + kronecker_delta(a, c) * r_dp(l, m, b) + kronecker_delta(a, b) * r_dp(l, m, c)) &
                                                          - 0.16666666667_dp * (r_dp(k, l, 1) * r_dp(l, m, 1) + r_dp(k, l, 2) * r_dp(l, m, 2) + r_dp(k, l, 3) * r_dp(l, m, 3)) * (kronecker_delta(b, c) * r_dp(n, k, a) + kronecker_delta(a, c) * r_dp(n, k, b) + kronecker_delta(a, b) * r_dp(n, k, c)) &
                                                          - 0.16666666667_dp * (r_dp(n, k, 1) * r_dp(l, m, 1) + r_dp(n, k, 2) * r_dp(l, m, 2) + r_dp(n, k, 3) * r_dp(l, m, 3)) * (kronecker_delta(b, c) * r_dp(k, l, a) + kronecker_delta(a, c) * r_dp(k, l, b) + kronecker_delta(a, b) * r_dp(k, l, c)) 
                enddo
              enddo
            enddo
          enddo
        ...
        do ifreq = 1, kubo_nfreq
          ...
          do a = 1, 3
            do b = 1, 3
              didi_polar_k_list(a, b, ifreq) = didi_polar_k_list(a, b, ifreq) + freqfactor_mn * r_dp(m, n, a) * r_dp(n, m, b) 
              do c = 1, 3
                diquad_polar_k_list(a, b, c, ifreq) = diquad_polar_k_list(a, b, c, ifreq) + freqfactor_mn * r_dp(m, n, a) * rr(n, m, b, c) 
                do e = 1, 3
                  quadquad_polar_k_list(a, b, c, e, ifreq) = quadquad_polar_k_list(a, b, c, e, ifreq) + freqfactor_mn * 0.5_dp * (rr(m, n, a, b) * rr(n, m, c, e) + rr(m, n, c, e) * rr(n, m, a, b))
                  dioctu_polar_k_list(a, b, c, e, ifreq) = dioctu_polar_k_list(a, b, c, e, ifreq) + freqfactor_mn * r_dp(m, n, a) * rrr(n, m, b, c, e)  
                enddo
              enddo
            enddo
          enddo
